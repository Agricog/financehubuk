  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-3">Free UK Mortgage Calculator 2025</h1>
          <p className="text-lg text-gray-600">Calculate your monthly mortgage payments, total interest, and affordability instantly. Accurate estimates for first-time buyers, remortgaging, and property investment.</p>
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-12">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Property Price: £{propertyPrice.toLocaleString()}</label>
                <input type="range" min="50000" max="2000000" step="10000" value={propertyPrice} onChange={(e) => { setPropertyPrice(parseFloat(e.target.value)); calculateMortgage() }} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer" aria-label="Property price slider" />
                <input type="number" value={propertyPrice} onChange={(e) => { setPropertyPrice(parseFloat(e.target.value) || 0); calculateMortgage() }} className="w-full mt-2 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" placeholder="Enter property price" aria-label="Property price input" />
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Deposit: £{deposit.toLocaleString()} ({((deposit/propertyPrice)*100).toFixed(1)}%)</label>
                <input type="range" min="0" max={propertyPrice * 0.5} step="5000" value={deposit} onChange={(e) => { setDeposit(parseFloat(e.target.value)); calculateMortgage() }} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer" aria-label="Deposit slider" />
                <input type="number" value={deposit} onChange={(e) => { setDeposit(parseFloat(e.target.value) || 0); calculateMortgage() }} className="w-full mt-2 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" placeholder="Enter deposit amount" aria-label="Deposit input" />
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Interest Rate (Annual %): {interestRate.toFixed(2)}%</label>
                <input type="range" min="0.5" max="10" step="0.1" value={interestRate} onChange={(e) => { setInterestRate(parseFloat(e.target.value)); calculateMortgage() }} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer" aria-label="Interest rate slider" />
                <input type="number" value={interestRate} onChange={(e) => { setInterestRate(parseFloat(e.target.value) || 0); calculateMortgage() }} step="0.1" className="w-full mt-2 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" placeholder="Enter interest rate" aria-label="Interest rate input" />
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Loan Term (Years): {loanTerm}</label>
                <input type="range" min="5" max="40" step="1" value={loanTerm} onChange={(e) => { setLoanTerm(parseFloat(e.target.value)); calculateMortgage() }} className="w-full h-2 bg-gray-300 rounded-lg appearance-none cursor-pointer" aria-label="Loan term slider" />
                <input type="number" value={loanTerm} onChange={(e) => { setLoanTerm(parseFloat(e.target.value) || 0); calculateMortgage() }} className="w-full mt-2 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" placeholder="Enter loan term in years" aria-label="Loan term input" />
              </div>

              <button onClick={calculateMortgage} className="w-full bg-primary-500 hover:bg-primary-600 text-white font-semibold py-3 rounded-lg transition">Calculate</button>
            </div>
              ...
              {results ? (
                <div className="bg-gradient-to-br from-primary-50 to-primary-100 rounded-lg p-8 space-y-4">
                  ...
                  <div className="bg-white rounded-lg p-4 space-y-3">
                    <div className="flex justify-between"><span className="text-gray-700">Loan Amount:</span><span className="font-semibold">£{parseFloat(results.loanAmount).toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
        ...
        <div className="space-y-12">
          ...
          <section>
            ...
            <div className="space-y-4">
              {[
                { q: "How much deposit do I need for a UK mortgage?", a: "Most lenders require a minimum deposit of 5-10% of the property value. However, larger deposits (15-20%) typically secure better interest rates and reduce your monthly payments. A 25%+ deposit unlocks the best deals." },
                { q: "How much can I borrow?", a: "Lenders typically allow borrowing of 4–4.5 times your annual gross income, subject to affordability checks. Use our calculator to get a personalized estimate based on your deposit and income." },
                { q: "Can I get a mortgage with poor credit?", a: "Yes, but with limitations. Poor credit means higher interest rates, smaller loans, and stricter terms. Improve your credit score before applying—even a 50-point increase can save you thousands in interest." },
                { q: "How are monthly payments calculated?", a: "Payments are calculated using an amortization formula based on: Principal (loan amount), interest rate (annual %), and loan term (years). Each payment gradually pays off both principal and interest." },
                { q: "What's the average UK mortgage rate in 2025?", a: "Fixed rates typically range 4.5–5.5%; variable rates 5–6%. Rates depend on lender, deposit size, credit score, and market conditions. Always compare multiple offers." },
                { q: "What is LTV (Loan-to-Value)?", a: "LTV is your loan amount divided by the property value (as a percentage). Lower LTV = better interest rates. Example: £180,000 loan on £250,000 property = 72% LTV." },
                { q: "Can I repay my mortgage early?", a: "Yes, most mortgages allow early repayment, but you may face early repayment charges (typically 1-5% of outstanding balance). Check your mortgage document for terms." },
                { q: "What's an interest-only mortgage?", a: "With interest-only mortgages, you pay only the interest each month. The full principal is due at the end of the term. Common for buy-to-let or short-term ownership." },
                { q: "Are mortgage calculators accurate?", a: "Calculators provide accurate estimates based on standard formulas, but actual terms depend on your lender, credit score, and personal circumstances. Use results as a guide, not a guarantee." },
                { q: "Do I need life insurance for a mortgage?", a: "Lenders won't require it, but it's highly recommended. Payment protection insurance (PPI) or life insurance ensures your family can pay the mortgage if you pass away or become unable to work." },
                { q: "What is a tracker mortgage?", a: "A tracker mortgage's interest rate 'tracks' the Bank of England base rate plus a lender margin. When base rates rise, so does your rate. Good if you expect rates to fall." },
                { q: "How do I choose a lender?", a: "Compare interest rates, fees, product flexibility, and customer reviews. Use comparison sites (MoneySuperMarket, Confused.com) or work with a mortgage broker for access to exclusive deals." },
                { q: "How long does mortgage approval take?", a: "Typically 1–4 weeks, depending on the lender and your circumstances. First-time buyers may take longer due to additional checks. Start the process early." },
                { q: "Can self-employed people get mortgages?", a: "Yes, but you'll need to provide 2-3 years of accounts, tax returns, and proof of business income. Some lenders are more flexible than others—shop around." },
                { q: "What is APR?", a: "APR (Annual Percentage Rate) includes the interest rate plus mandatory fees (arrangement, valuation, etc.). It gives a true picture of the annual cost of your mortgage." }
              ].map((faq, idx) => (
